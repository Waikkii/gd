...
async function takePostRequest(type){
    ...
    switch (type) {
        case 'index':
            ...
            break;
        case 'doTask':
            body = `appid=anniversary-celebra&functionId=jd_interaction_prod&body={"parentId":"${$.oneTask.parentId}","taskId":"${$.oneTask.taskId}","apiMapping":"/api/task/doTask"}&t=${Date.now()}&loginType=2`
            break;
        case 'getReward':
            body = `appid=anniversary-celebra&functionId=jd_interaction_prod&body={"parentId":"${$.oneTask.parentId}","taskId":"${$.oneTask.taskId}","timeStamp":${$.timeStamp},"apiMapping":"/api/task/getReward"}&t=${Date.now()}&loginType=2`;
            break;
        default:
            console.log(`错误${type}`);
    }
