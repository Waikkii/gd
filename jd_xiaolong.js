/*
骁龙品牌日
不开卡，只做浏览任务和抽奖,每天若干豆，默认助力作者助力，如不助力helpAuthor设置为false即可，感谢！
活动地址：https://lzdz4-isv.isvjcloud.com/dingzhi/xiaolong/collectcard/activity/4095893?activityId=901080701
活动日期：2021年08月13日-2021年08月25日
cron 12 7,22 * * *
*/
const $ = new Env("骁龙品牌日");
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = ['f52acb7e11f34616b7d44bc67880ca5f']
            ...
            $.activityUrl = `https://lzdz4-isv.isvjcloud.com/dingzhi/xiaolong/collectcard/activity?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await xl();
        }
    }
...
async function xl() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await task('xiaolong/collectcard/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}&picId=`)
            if ($.activityContent) {
                await task('xiaolong/collectcard/myInfo', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`)
                for (const vo of $.taskList) {
                    if(vo.taskid!='share2help' && vo.taskid!='joinvip' && vo.taskid!='joinvip2'){
                        if(vo.lastNum === vo.maxNeed){
                            console.log(`任务：${vo.taskname}，已完成`);
                            continue;
                        }
                        if(vo.lastNum !== vo.maxNeed){
                            console.log(`任务：${vo.taskname}，去完成`);
                            await $.wait(2000);
                            await task('xiaolong/collectcard/doTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&taskId=${vo.taskid}`)
                        }
                    }
                }
                $.log("去抽奖")
                if ($.activityContent.openCardStatus === 0){
                    $.log(`共有:${$.drawChance}次抽奖`)
                    for (let i = 0; i < $.drawChance; i++) {
                        await $.wait(2000);
                        await task('xiaolong/collectcard/luckydraw', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&type=0`)
                    }
                }else{
                    $.log("你还不是会员,不能抽奖")
                }
                        ...
                        if (data.result) {
                            switch (function_id) {
                                ...
                                case 'wxActionCommon/getUserInfo':
                                    ...
                                    break;
                                case 'xiaolong/collectcard/activityContent':
                                    $.log(`开启【${data.data.activityName}】活动`)
                                    $.log("-------------------")
                                    $.activityId = data.data.activityId
                                    $.activityContent = data.data;
                                    $.actorUuid = data.data.shareUser;
                                    break;
                                case 'xiaolong/collectcard/helpFriend':
                                    if (data.result) {
                                        console.log(data.data.helpFriendMsg)
                                    }
                                    break;
                                case 'xiaolong/collectcard/myInfo':
                                    if (data.result) {
                                        $.taskList = data.data.task;
                                        $.drawChance = data.data.drawChance
                                    }
                                    break;
                                case 'xiaolong/collectcard/doTask':
                                    if (data.result) {
                                        console.log(`获得：${data.data.beanNum}豆`)
                                    }
                                    break;
                                case 'xiaolong/collectcard/luckydraw':
                                    if (data.result) {
