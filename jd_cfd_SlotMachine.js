!function (t, r) {
  ...
  return function () {
    ...
    var e = t, i = e.lib, n = i.StreamCipher, o = e.algo, s = o.RC4 = n.extend({
      _doReset: function () {
        for (var t = this._key, r = t.words, e = t.sigBytes, i = this._S = [], n = 0; n < 256; n++) i[n] = n;
        for (var n = 0, o = 0; n < 256; n++) {
          var s = n % e, a = r[s >>> 2] >>> 24 - s % 4 * 8 & 255;
          o = (o + i[n] + a) % 256;
          var c = i[n];
          i[n] = i[o], i[o] = c
        }
    ...
    function r() {
      ...
      for (var e = 0; e < 8; e++) {
        var i = t[e] + r[e], n = 65535 & i, o = i >>> 16, s = ((n * n >>> 17) + n * o >>> 15) + o * o,
    ...
    var e = t, i = e.lib, n = i.StreamCipher, o = e.algo, s = [], a = [], c = [], h = o.Rabbit = n.extend({
      _doReset: function () {
        ...
        if (e) {
          var s = e.words, a = s[0], c = s[1], h = 16711935 & (a << 8 | a >>> 24) | 4278255360 & (a << 24 | a >>> 8),
            l = 16711935 & (c << 8 | c >>> 24) | 4278255360 & (c << 24 | c >>> 8), f = h >>> 16 | 4294901760 & l,
            u = l << 16 | 65535 & h;
    ...
    var e = t, i = e.lib, n = i.StreamCipher, o = e.algo, s = [], a = [], c = [], h = o.RabbitLegacy = n.extend({
      _doReset: function () {
...
const fs = require('fs');
if (!fs.existsSync('cfd.json')) {
  fs.writeFileSync('cfd.json', '')
}
const $ = new Env("京喜财富岛");
...
let cookiesArr = [], cookie = '', slotTimes = 0, message = '';
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookiesArr[i]) {
      ...
      await main();
      await goods();
      message ? await notify.sendNotify("财富岛兑换新品", message) : ''
    }
    break
  }
})()
...
function main() {
  ...
  return new Promise(async resolve => {
    try {
      if (slotTimes !== 10) {
        let url = `https://m.jingxi.com/jxcfd/consume/SlotMachine?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=138631.26.55&dwType=0&dwQueryPri=1&_stk=_cfd_t%2CbizCode%2CdwEnv%2CdwQueryPri%2CdwType%2Cptag%2Csource%2CstrZone&_ste=1&_=${Date.now()}&sceneval=2`
        let h5st = decrypt(Date.now(), '_cfd_t,bizCode,ddwConfVersion,dwEnv,ptag,source,strCouponPool,strGoodsPool,strZone', '', url)
        url += '&h5st=' + h5st
        $.get({
          url: url,
          headers: {
          ...
          console.log(data)
          if (data['strPrizeName'] === '金元宝') {
            console.log("恭喜这个B！")
            resolve();
          } else if (data['strPrizeName']) {
            console.log(`抽到了${data['strPrizeName']}`)
            await $.wait(4000)
            await main()
          } else {
            resolve();
...
function goods() {
  return new Promise(resolve => {
    let url = `https://m.jingxi.com/jxcfd/task/QueryAllConfig?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=138631.26.55&_stk=_cfd_t%2CbizCode%2CdwEnv%2Cptag%2Csource%2CstrZone&_ste=1&_=${Date.now()}&sceneval=2`
    let h5st = decrypt(Date.now(), '_cfd_t,bizCode,dwEnv,ptag,source,strZone', '', url)
    url += '&h5st=' + h5st
    $.get({
      url: url,
      headers: {
      ...
      try {
        let temp = JSON.parse(fs.readFileSync('cfd.json', 'utf-8'))
        let len = temp.length
        data = JSON.parse(data)
        for (let j of data.sData.exchangeCfg[0].materialData) {
          if (j.levelstatus) {
            console.log(j.name)
            if (!temp.includes(j.name)) {
              console.log('add:', j.name)
              temp.push(j.name)
              message += j.name + '\n'
            }
          }
        ...
        if (temp.length !== len) {
          writeFile(JSON.stringify(temp), 'cfd.json')
        }
      } catch (e) {
        console.log(e)
      } finally {
...
function writeFile(text, filename = 'a.json') {
