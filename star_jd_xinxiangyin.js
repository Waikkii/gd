...
async function main() {
    ...
    let drawScore = $.activityData.drawScore;
    console.log(`可以寻宝次数:${drawScore}`);
    await takePostRequest('drawContent');
    await $.wait(1000);
    let time = 0;
    for (let i = 0; i < drawScore && time < 10; i++) {
        console.log(`\n执行一次寻宝`);
...
async function doTask(){
    ...
    for (let key in activeTask) {
        if(activeTask[key].allStatus){
            console.log(`任务：${key}，已完成`)
            continue;
        }
        if(key === 'addSku'){
            console.log(`任务：${key},去执行`);
            $.oneTask = {'type':21,'value':21};
        ...
        if(key === 'followSku'){
            console.log(`任务：${key},去执行`);
            $.oneTask = {'type':22,'value':22};
            await takePostRequest('saveTask');
            await $.wait(2000);
            continue;
        }
        $.taskList = activeTask[key].data;
        for (let i = 0; i < $.taskList.length; i++) {
            $.oneTask = $.taskList[i];
            if($.oneTask.status !== 0){
                continue;
            }
            console.log(`任务：${key},${$.oneTask.name},去执行`);
            await takePostRequest('saveTask');
            await $.wait(2000);
            $.flag = true;
        }
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'activityContent':
            url = 'https://lzdz1-isv.isvjcloud.com/dingzhi/xinxiangyin/treasure/activityContent';
            ...
            break;
        case 'saveTask':
            url = 'https://lzdz1-isv.isvjcloud.com/dingzhi/xinxiangyin/treasure/saveTask';
            body = `activityId=${$.activityID}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.pin)}&taskType=${$.oneTask.type}&taskValue=${$.oneTask.value}`;
            break;
        case 'startDraw':
            let changeList = [1,2,3,4,5];
            let change = getRandomArrayElements(changeList,1)[0];
            url = 'https://lzdz1-isv.isvjcloud.com/dingzhi/xinxiangyin/treasure/startDraw';
            body = `activityId=${$.activityID}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.pin)}&change=${change}`;
            break;
        case 'drawContent':
        ...
        case 'saveTask':
            if (data.data && data.result && data.count === 0) {
                if(data.data.addScore){
                    console.log(`执行成功，增加${data.data.addScore}次寻宝次数`);
                }else{
                    console.log(`执行成功`);
                }
            } else {
...
async function getMyPing() {
    let url = 'https://lzdz1-isv.isvjcloud.com/customer/getMyPing';
...
function getPostRequest(url, body) {
    const headers = {
        ...
        'Referer' : `https://lzdz1-isv.isvjcloud.com/dingzhi/xinxiangyin/treasure/activity/${$.activityID}?activityId=${$.activityID}&shareUuid=${$.shareUuid}`,
...
function getHtml() {
    let config ={
        url:`https://lzdz1-isv.isvjcloud.com/dingzhi/xinxiangyin/treasure/activity/${$.activityID}?activityId=${$.activityID}&shareUuid=${$.shareUuid}`,
        headers: {
            'Host':'lzdz1-isv.isvjcloud.com',
...
function getWxCommonInfoToken () {
    const url = `https://lzdz1-isv.isvjcloud.com/wxCommonInfo/token`;
    const method = `POST`;
    const headers = {
        'X-Requested-With' : `XMLHttpRequest`,
        'Connection' : `keep-alive`,
        ...
        'Cookie' : cookie,
        'Host' : `lzdz1-isv.isvjcloud.com`,
        'Referer' : `https://lzdz1-isv.isvjcloud.com/dingzhi/xinxiangyin/treasure/activity/${$.activityID}?activityId=${$.activityID}`,
        'Accept-Language' : `zh-cn`,
        'Accept' : `application/json`
    };
    const body = ``;
